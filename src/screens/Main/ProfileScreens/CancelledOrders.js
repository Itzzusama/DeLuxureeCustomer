...
const CancelledOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res = await get(`order/customer/cancelled`);
      if (res.data.success) {
      ...
      console.error("Failed to fetch orders:", error.res.data.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getMoreOrders = async () => {
    if (order?.length > 0) {
      try {
        ...
        const res = await get(`order/customer/cancelled/${lastOrder}`);
        if (res.data.success) {
